    ...
    return (
        <section aria-labelledby="task-history-title">
            <header style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 16 }}>
                <div>
                    <h3 id="task-history-title" className="title" style={{ fontSize: '1.2rem' }}>Request History</h3>
                ...
                <Tip content="Clear the local task list (does not cancel remote tasks).">
                    <button type="button" className="btn-danger" onClick={onClearAll} disabled={busy || tasks.length === 0} aria-label="Clear all tasks" style={{ padding: '8px 12px' }}>
            ...
            {tasks.length === 0 ? (
                <div style={{ padding: 32, textAlign: 'center', color: 'var(--text-tertiary)', background: 'rgba(0,0,0,0.1)', borderRadius: 12 }}>
                    <Box aria-hidden="true" size={24} style={{ opacity: 0.3, marginBottom: 8 }} />
                    <div style={{ fontSize: '0.9rem', fontWeight: 500 }}>No active tasks</div>
                    <p style={{ marginTop: '8px', fontSize: '0.8rem' }}>Generate a model using the form on the left to see it here.</p>
                </div>
            ) : (
                <ul className="scroll-area" style={{ maxHeight: 300, display: "grid", gap: 8, padding: '0 4px 0 0', margin: 0, listStyle: 'none' }} aria-label="Task list">
                                ...
                                <div style={{ display: "flex", justifyContent: "space-between", gap: 10, alignItems: 'center' }}>
                                    <div style={{ minWidth: 0, display: 'flex', alignItems: 'center', gap: 10 }}>
                                        <div style={{ padding: 8, borderRadius: 8, background: 'rgba(255,255,255,0.05)' }}>
                                            {t.type === "text" ? <Box aria-hidden="true" size={16} /> : <ImageIcon aria-hidden="true" size={16} />}
                                        </div>
                                        <div>
