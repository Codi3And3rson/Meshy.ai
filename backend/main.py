...
ALLOWED_DOWNLOAD_HOSTS_SET = frozenset(ALLOWED_DOWNLOAD_HOSTS)
ALLOWED_DOWNLOAD_HOSTS_SUFFIX_TUPLE = tuple("." + h for h in ALLOWED_DOWNLOAD_HOSTS)
...
def _validate_download_url(url: str) -> None:
    ...
    if host not in ALLOWED_DOWNLOAD_HOSTS_SET and not host.endswith(
        ALLOWED_DOWNLOAD_HOSTS_SUFFIX_TUPLE
    ):
        raise HTTPException(
            status_code=403,
