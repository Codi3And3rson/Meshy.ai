...
def normalize_task(raw: dict) -> dict:
    ...
    if not model_urls:
        if "result" in raw and isinstance(raw["result"], dict):
            model_urls = pick_model_urls(raw["result"])
        if not model_urls and "output" in raw and isinstance(raw["output"], dict):
            model_urls = pick_model_urls(raw["output"])
    ...
    if not (isinstance(thumb, str) and thumb.startswith("http")):
        thumb = None
        if "result" in raw and isinstance(raw["result"], dict):
            t2 = raw["result"].get("thumbnail_url")
            if isinstance(t2, str) and t2.startswith("http"):
                thumb = t2
        if thumb is None and "output" in raw and isinstance(raw["output"], dict):
            t2 = raw["output"].get("thumbnail_url")
            if isinstance(t2, str) and t2.startswith("http"):
                thumb = t2
