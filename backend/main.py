...
MODEL_URL_KEYS = ("glb", "fbx", "obj", "usdz", "mtl", "pre_remeshed_glb")
PREFERRED_MODEL_ORDER = ("glb", "fbx", "obj", "usdz", "pre_remeshed_glb")
FALLBACK_MODEL_PATTERNS = (("model_url", "unknown"), ("glb", "glb"), ("gltf", "gltf"))
...
def normalize_task(raw: dict) -> dict:
    ...
    model = None
    for fmt in PREFERRED_MODEL_ORDER:
    ...
    if model is None:
        for key, fmt in FALLBACK_MODEL_PATTERNS:
            v = raw.get(key)
            if isinstance(v, str) and v.startswith("http"):
