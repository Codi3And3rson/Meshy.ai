...
def _extract_bearer_token(authorization: str | None) -> str | None:
    ...
    if a.lower().startswith("bearer "):
        # Check if the remaining part after "bearer " actually exists and has non-whitespace characters
        parts = a.split(" ", 1)
        if len(parts) > 1:
            tok = parts[1].strip()
            return tok or None
        return None
    # Also handle the edge case where "bearer" has trailing spaces stripped previously but only contains "bearer"
    if a.lower() == "bearer":
        return None
    return a
