...
_raw_meshy_base = os.getenv("MESHY_BASE", "https://api.meshy.ai").rstrip("/")
_parsed_meshy_base = urlparse(_raw_meshy_base)

if _parsed_meshy_base.scheme not in ("http", "https"):
    raise ValueError("MESHY_BASE must use http or https scheme")

_allowed_meshy_hosts = {"api.meshy.ai", "localhost", "127.0.0.1"}
if _parsed_meshy_base.hostname not in _allowed_meshy_hosts:
    raise ValueError(
        f"Untrusted MESHY_BASE host: {_parsed_meshy_base.hostname}. Allowed hosts: {', '.join(_allowed_meshy_hosts)}"
    )

MESHY_BASE = _raw_meshy_base
