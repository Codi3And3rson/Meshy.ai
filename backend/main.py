    ...
    while True:
        try:
            ...
            if attempt < MAX_RETRIES:
                ...
                await asyncio.sleep(delay)
                continue
            logger.error(f"Upstream network error talking to Meshy: {exc!s}")
            raise HTTPException(
                status_code=502, detail="Upstream network error talking to Meshy"
            ) from exc
...
async def download_proxy(
    ...
    try:
        ...
        logger.error(f"Upstream download error: {exc!s}")
        raise HTTPException(status_code=502, detail="Upstream download error") from exc
